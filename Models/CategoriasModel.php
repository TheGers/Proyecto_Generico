<?php 
	...
	class CategoriasModel extends Mysql
	{
		...
		public function inserCategoria(string $nombre, string $descripcion, string $portada, int $status){
			...
			if(empty($request))
			{
				$query_insert  = "INSERT INTO tbl_categoria(nombre,descripcion,portada,status) VALUES(?,?,?,?)";
		...
		public function updateCategoria(int $idcategoria, string $categoria, string $descripcion, string $portada, int $status){
			...
			$sql = "SELECT * FROM tbl_categoria WHERE nombre = '{$this->strCategoria}' AND idcategoria != $this->intIdcategoria";
			$request = $this->select_all($sql);
		...
		{
			$this->intIdcategoria = $idcategoria;
			$sql = "SELECT * FROM tbl_productos WHERE categoriaid = $this->intIdcategoria";
			$request = $this->select_all($sql);
			if(empty($request))
			{
				$sql = "UPDATE tbl_categoria SET status = ? WHERE idcategoria = $this->intIdcategoria ";
				$arrData = array(0);
				$request = $this->update($sql,$arrData);
				if($request)
				{
